...
class Analytics implements HttpPostActionInterface, CsrfAwareActionInterface
{
    ...
    public function execute(): Json
    {
        ...
        try {
            ...
            $data = $this->storeContext->execute((string)($payload['storeCode'] ?? ''), fn (): array => $this->analytics->record($payload));
            return $result->setHttpResponseCode($data['status'] === 'error' ? 422 : 200)->setData($data);
        } catch (AuthorizationException) {
            return $result->setHttpResponseCode(403)->setData(['status' => 'error', 'message' => 'The analytics event could not be verified.']);
        } catch (\Throwable) {
            return $result->setHttpResponseCode(400)->setData(['status' => 'error', 'message' => 'The analytics event could not be recorded.']);
        }
    }
}
