...
class Config
{
    ...
    /** Magento storefront URL used by Node for signed internal calls. */
    const XML_PATH_MAGENTO_SECURE_BASE_URL = 'web/secure/base_url';
    const XML_PATH_MAGENTO_UNSECURE_BASE_URL = 'web/unsecure/base_url';
    ...
    const XML_PATH_GEMINI_IMAGE_MODEL = 'afd_ai/general/gemini_image_model';
    ...
    const XML_PATH_AGENT_MAX_HISTORY_TOKENS = 'afd_ai/agent/max_history_tokens';
    const XML_PATH_AGENT_MAX_TOOL_CONTEXT_TOKENS = 'afd_ai/agent/max_tool_context_tokens';
    ...
    const XML_PATH_ATTACHMENT_MAX_TOTAL_BYTES = 'afd_ai/attachments/max_total_image_bytes';
    const XML_PATH_ATTACHMENT_MAX_TOTAL_ENCODED_BYTES = 'afd_ai/attachments/max_total_encoded_bytes';
    const XML_PATH_ATTACHMENT_MAX_TOTAL_PIXELS = 'afd_ai/attachments/max_total_pixels';
    const XML_PATH_ATTACHMENT_VISION_CONCURRENCY = 'afd_ai/attachments/vision_concurrency';
    ...
    const XML_PATH_VOICE_GEMINI_MODEL = 'afd_ai/voice/gemini_model';
     ...
     * @param GatewaySecretManager $gatewaySecretManager
     */
    public function __construct(
        ScopeConfigInterface $scopeConfig,
        EncryptorInterface $encryptor,
        private readonly ?GatewaySecretManager $gatewaySecretManager = null
    ) {
    ...
    public function getChatServerUrl($storeId = null): string
    {
        ...
        $baseUrl = $this->getMagentoBaseUrl($storeId);
        if ($baseUrl === '') {
            return '';
        }
        ...
        $path = $basePath === '' ? '/ai-gateway/' : '/' . $basePath . '/ai-gateway/';

        return $webSocketScheme . '://' . $authority . $path;
    }

    /**
     * Resolve the current store's public Magento base URL.  This is deliberately
     * read from Magento's Store configuration so a deployment can change its
     * domain without changing the Node environment.
     */
    public function getMagentoBaseUrl($storeId = null): string
    {
        $scope = ScopeInterface::SCOPE_STORE;
        $url = (string)$this->scopeConfig->getValue(self::XML_PATH_MAGENTO_SECURE_BASE_URL, $scope, $storeId);
        if ($url === '') {
            $url = (string)$this->scopeConfig->getValue(self::XML_PATH_MAGENTO_UNSECURE_BASE_URL, $scope, $storeId);
        }
    ...
    {
        $imageModelPath = $this->getProvider($storeId) === 'gemini'
            ? self::XML_PATH_GEMINI_IMAGE_MODEL
    ...
    public function getAgentConfig($storeId = null): array
    {
        return [
            ...
            'max_history_tokens' => $this->getIntValue(self::XML_PATH_AGENT_MAX_HISTORY_TOKENS, 12000, 512, 64000, $storeId),
            'max_tool_context_tokens' => $this->getIntValue(self::XML_PATH_AGENT_MAX_TOOL_CONTEXT_TOKENS, 6000, 256, 24000, $storeId),
    ...
    public function getAttachmentConfig($storeId = null): array
    {
        return [
            ...
            'max_total_image_bytes' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_MAX_TOTAL_BYTES,
                6291456,
                262144,
                8388608,
                $storeId
            ),
            'max_total_encoded_bytes' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_MAX_TOTAL_ENCODED_BYTES,
                6291456,
                524288,
                6291456,
                $storeId
            ),
            'max_total_pixels' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_MAX_TOTAL_PIXELS,
                30000000,
                1000000,
                50000000,
                $storeId
            ),
            'vision_concurrency' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_VISION_CONCURRENCY,
                4,
                1,
                32,
                $storeId
            ),
            'cost_units_per_minute' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_COST_UNITS,
                30,
                5,
                300,
                $storeId
            ),
            'network_cost_units_per_minute' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_NETWORK_COST_UNITS,
                120,
                10,
                1000,
                $storeId
            ),
            'global_cost_units_per_minute' => $this->getIntValue(
                self::XML_PATH_ATTACHMENT_GLOBAL_COST_UNITS,
                1200,
                100,
                10000,
                $storeId
    ...
    {
        $provider = $this->getProvider($storeId);
        $isOpenAiProvider = $provider === 'openai';
        $transcriptionModel = $provider === 'gemini'
        ...
        if ($provider === 'gemini' && trim($transcriptionModel) === '') {
            $transcriptionModel = (string)$this->getModel($storeId);
        }
        ...
        return [
            ...
            'live' => [
                'enabled' => $isOpenAiProvider && $this->scopeConfig->isSetFlag(
                ...
                'api_key' => $isOpenAiProvider
                    ? $this->getEncryptedValue(self::XML_PATH_VOICE_LIVE_OPENAI_API_KEY, $storeId)
                    : '',
                'model' => $isOpenAiProvider
                    ? (string)$this->scopeConfig->getValue(
    ...
    /**
     * Return the Magento-owned HMAC secret for the Node configuration endpoint.
     */
    public function getNodeSyncSecret($storeId = null): string
    {
        return $this->gatewaySecretManager
            ? $this->gatewaySecretManager->getNodeSyncSecret()
            : $this->getEncryptedValue(self::XML_PATH_NODE_SYNC_SECRET, $storeId);
    }
    ...
    {
        return $this->gatewaySecretManager
            ? $this->gatewaySecretManager->getWebSocketTicketSecret()
            : $this->getEncryptedValue(self::XML_PATH_WS_TICKET_SECRET, $storeId);
    }
    ...
    public function getVisibleValue(string $path, $storeId = null): string
    {
        ...
        try {
            ...
            // A database copied from another Magento installation may contain
            // ciphertext encrypted with a different crypt key. Never put
            // undecodable binary into a JSON config push or provider header.
            return mb_check_encoding($decrypted, 'UTF-8')
                && !preg_match('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', $decrypted)
                ? $decrypted
                : '';
    ...
    {
        return $this->getVisibleValue($path, $storeId);
    }
