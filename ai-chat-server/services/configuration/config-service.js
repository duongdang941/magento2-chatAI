...
function normalizeModelCapabilities(source = {}) {
    const raw = source.capabilities && typeof source.capabilities === 'object' && !Array.isArray(source.capabilities)
        ? source.capabilities
        : {};
    const legacyImage = source.supports_images ?? source.supportsImages;
    const legacyImageEnabled = normalizeBoolean(legacyImage, false);

    return {
        image_generation: normalizeBoolean(
            raw.image_generation ?? raw.create_edit_image ?? (legacyImageEnabled ? true : undefined),
            true
        ),
        video_generation: normalizeBoolean(raw.video_generation ?? raw.create_edit_video, false),
        voice_dictation: normalizeBoolean(raw.voice_dictation ?? raw.voice, false)
    };
}
...
function normalizeProviderModels(value) {
    ...
    return value.map((model) => {
        ...
        if (!id) return null;
        const capabilities = normalizeModelCapabilities(source);
        ...
        const supportsImages = capabilities.image_generation;
        ...
        return {
            ...
            max_output_tokens: maxOutputConfigured
                ? clampInteger(rawMaxOutputTokens, 0, 256, 1000000)
                : null,
            ...
            image_model: supportsImages ? readString(source.image_model ?? source.imageModel) : '',
            voice_model: capabilities.voice_dictation
                ? readString(source.voice_model ?? source.voiceModel)
                : ''
        };
    }).filter(Boolean);
}
...
function normalizeImageGeneration(value, provider, selectedModel = null, config = {}) {
    ...
    const modelAllowsImages = selectedModel
        ? selectedModel.capabilities?.image_generation === true
        : true;
    const transport = selectedModel
        ? (modelAllowsImages ? modelTransport || defaultImageTransport({ ...config, provider }) : '')
        : (hasOwn(source, 'transport')
            ? explicitTransport
            : explicitTransport || defaultImageTransport({ ...config, provider }));
    const configuredModel = selectedModel
        ? readString(selectedModel.image_model)
        : (hasOwn(source, 'model') ? readString(source.model) : '');

    return {
        enabled: selectedModel
            ? modelAllowsImages
            : (hasOwn(source, 'enabled') ? Boolean(source.enabled) : process.env.IMAGE_GENERATION_ENABLED !== '0'),
...
function normalizeAgent(value, selectedModel = null) {
    const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
    const modelMaxOutputTokens = selectedModel?.max_output_tokens_configured === true
        ? clampInteger(selectedModel.max_output_tokens, 2048, 256, 1000000)
        : null;
    ...
    return {
        ...
        max_output_tokens: modelMaxOutputTokens ?? clampInteger(source.max_output_tokens, 2048, 256, 1000000),
...
function normalizeVoice(value, provider, fallbackModel = '', selectedModel = null) {
    const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
    const liveSource = source.live && typeof source.live === 'object' && !Array.isArray(source.live)
        ? source.live
        : {};

    const modelAllowsVoice = selectedModel
        ? selectedModel.capabilities?.voice_dictation === true
        : true;
    const configuredTranscriptionModel = selectedModel
        ? readString(selectedModel.voice_model)
        : readString(source.transcription_model);

    return {
        enabled: selectedModel
            ? modelAllowsVoice
            : (hasOwn(source, 'enabled') ? Boolean(source.enabled) : true),
        // A Voice capability does not require a second model ID in the
        // editor.  When the merchant leaves it empty, retain the established
        // provider-safe default instead of synchronizing an unusable blank.
        transcription_model: configuredTranscriptionModel || (
            provider === 'gemini'
...
export function normalizeConfig(config = {}) {
    ...
    const normalized = {
        ...
        agent: normalizeAgent(selectedConfig.agent, selectedModel),
        ...
        voice: normalizeVoice(selectedConfig.voice, provider, model, selectedModel),
