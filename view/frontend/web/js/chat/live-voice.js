...
(function (modules) {
    ...
    const AUDIO_WORKLET_SOURCE = `class AfdVoiceLevelProcessor extends AudioWorkletProcessor { process(inputs) { const channel = inputs[0]?.[0]; if (channel?.length) { let sum = 0; for (let i = 0; i < channel.length; i += 1) sum += channel[i] * channel[i]; this.port.postMessage(Math.min(1, Math.sqrt(sum / channel.length) * 4)); } return true; } } registerProcessor('afd-voice-level', AfdVoiceLevelProcessor);`;
    async function attachAudioWorkletMonitor(instance, stream) {
        const Ctor = window.AudioContext || window.webkitAudioContext;
        if (!Ctor || !Ctor.prototype.audioWorklet) return;
        let context; let url;
        try {
            context = new Ctor();
            url = URL.createObjectURL(new Blob([AUDIO_WORKLET_SOURCE], {type: 'application/javascript'}));
            await context.audioWorklet.addModule(url);
            const source = context.createMediaStreamSource(stream);
            const node = new AudioWorkletNode(context, 'afd-voice-level');
            const silent = context.createGain(); silent.gain.value = 0;
            node.port.onmessage = event => { instance.liveVoiceInputLevel = Math.max(0, Math.min(1, Number(event.data) || 0)); };
            source.connect(node); node.connect(silent); silent.connect(context.destination); await context.resume();
            instance.liveVoiceMicStream = stream; instance.liveVoiceAudioContext = context; instance.liveVoiceAudioWorkletNode = node; instance.liveVoiceAudioWorkletUrl = url;
        } catch { if (context) await context.close().catch(() => {}); if (url) URL.revokeObjectURL(url); }
    }
    function closeAudioWorkletMonitor(instance) {
        instance.liveVoiceAudioWorkletNode?.port?.close?.(); instance.liveVoiceAudioWorkletNode?.disconnect?.(); instance.liveVoiceAudioWorkletNode = null;
        instance.liveVoiceAudioContext?.close?.().catch?.(() => {}); instance.liveVoiceAudioContext = null;
        if (instance.liveVoiceAudioWorkletUrl) URL.revokeObjectURL(instance.liveVoiceAudioWorkletUrl); instance.liveVoiceAudioWorkletUrl = '';
        instance.liveVoiceMicStream?.getTracks?.().forEach(track => track.stop()); instance.liveVoiceMicStream = null; instance.liveVoiceInputLevel = 0;
    }
    function randomId() {
    ...
    function closePeerConnection(instance) {
        ...
        closeAudioWorkletMonitor(instance);
        ...
        return {
            ...
            async toggleLiveVoice() {
                ...
                try {
                    ...
                    if (!this.socket || !this.wsConnected) throw new Error('The secure chat connection is unavailable. Live Voice is a separate OpenAI WebRTC call and is not Gemini dictation.');
            ...
            async connectLiveVoiceSession(data) {
                ...
                try {
                    ...
                    await attachAudioWorkletMonitor(this, stream);
                    peer.ontrack = event => {
