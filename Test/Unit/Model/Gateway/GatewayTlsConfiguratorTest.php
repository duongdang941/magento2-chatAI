...
class GatewayTlsConfiguratorTest extends TestCase
{
    public function testUsesDetectedValetCaFileWithoutDisablingTlsVerification(): void
    {
        ...
        try {
            ...
            (new GatewayTlsConfigurator($homeDirectory))->configure($curl, 'https://afd.test/ai-gateway');
        } finally {
            @unlink($caFile);
            @rmdir($caDirectory);
    ...
    public function testUsesSystemCaBundleWhenValetIsNotInstalled(): void
    {
        ...
        (new GatewayTlsConfigurator(sys_get_temp_dir() . '/afd-ai-no-valet'))->configure($curl, 'https://gateway.example.com');
    }

    public function testDoesNotUseValetCaForPublicGateway(): void
    {
        $homeDirectory = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'afd-ai-home-' . bin2hex(random_bytes(4));
        $caDirectory = $homeDirectory . '/.config/valet/CA';
        self::assertTrue(mkdir($caDirectory, 0700, true));
        ...
        try {
            ...
            (new GatewayTlsConfigurator($homeDirectory))->configure($curl, 'https://socket.bingxgames.com');
        } finally {
            @unlink($caFile);
            @rmdir($caDirectory);
            @rmdir(dirname($caDirectory));
