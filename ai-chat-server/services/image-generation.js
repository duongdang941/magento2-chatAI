...
function resolveImageProviderConfig(config = {}) {
    ...
    if (!['cockpit', 'openai'].includes(provider)) {
        throw new Error('Image generation is currently available through Cockpit or OpenAI only.');
    }
    ...
    const model = readString(
        imageConfig.model,
        process.env[`${envPrefix}_IMAGE_MODEL`] || DEFAULT_IMAGE_MODEL
    );
...
function generatedImagePublicBaseUrl() {
...
async function persistGeneratedImage(image, outputFormat) {
    ...
    return {
        url: `${generatedImagePublicBaseUrl()}/media/afd-ai/generated/${filename}`,
...
export async function generateImage({ prompt, ws, config = {}, signal = null, isCancelled = () => false } = {}) {
    ...
    try {
        ...
        const stored = await persistGeneratedImage(firstImage, payload.output_format || 'png');
