...
- CATALOG_CONTEXT is a private reference ledger from an older product grid, not current catalogue evidence and not a product result. Use it only to resolve an unambiguous follow-up that explicitly identifies a previously shown SKU/product_ref or singularly refers to one previously shown card (for example “this one” immediately after one card). Never use its names, prices, counts, options, or availability to compose a new product answer.
