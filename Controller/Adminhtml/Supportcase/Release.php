...
class Release extends Action implements HttpPostActionInterface
{
    ...
    public function execute(): ResultInterface
    {
        ...
        try {
            ...
            $this->takeoverService->release($caseId, (int)$admin->getId(), trim((string)$admin->getFirstName() . ' ' . (string)$admin->getLastName()));
            $this->messageManager->addSuccessMessage(__('Live chat ended. The customer can use AI again.'));
            if ($this->getRequest()->isAjax()) {
                /** @var Json $json */ $json = $this->resultFactory->create(ResultFactory::TYPE_JSON);
                return $json->setData(['status' => 'success', 'active' => false]);
            }
