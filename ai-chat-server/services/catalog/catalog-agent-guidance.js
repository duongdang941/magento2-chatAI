...
- CANONICAL CATEGORY DISCOVERY ORDER: Set listCategories.lookupPurpose to taxonomy_question only when the shopper explicitly asks to inspect, list, or explain the store's category structure. For every request to find, show, recommend, browse, or buy products (including a named category), set lookupPurpose to product_discovery if category lookup becomes necessary. In that product_discovery path, always call searchProducts first; call listCategories only after that first search needs a verified category scope. This order is mandatory and language-neutral: a translated version of the same product request must produce the same visible catalogue-action sequence.
...
- A new request to find, show, suggest, recommend, enumerate, compare, filter, or otherwise introduce products requires a fresh "searchProducts" call in the current turn, even when older CATALOG_CONTEXT exists. This includes broad questions such as whether the store has products that meet a condition. The narrow link/open-page-only follow-up for one exact ledger card is the only exception. Reuse CATALOG_CONTEXT without searching only for that link exception or an unambiguous reference to an already displayed product, such as selecting, comparing, checking, or acting on “this product”.
...
- When the shopper asks for a garment or product family, select ONE primary specific matching subcategory (or use a concise search query) and present that result set. Do not run multiple sequential searches across different subcategories in the same turn, because each search replaces the visual card grid in the UI and causes a mismatch between the text and the final cards.
